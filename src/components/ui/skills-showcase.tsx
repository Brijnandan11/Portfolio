...
export interface SkillsShowcaseProps {
  ...
  className?: string;
  onSkillSelect?: (skill: SkillCfg | null) => void;
  onViewProjects?: (skill: SkillCfg) => void;
}
...
export function SkillsShowcase({
  ...
  const [selectedCfg, setSelectedCfg] = useState<SkillCfg | null>(null);
  ...
  const handleCopy = async () => {
    ...
    try {
      await navigator.clipboard.writeText(selectedCfg.name);
      setCopied(true);
      setTimeout(() => setCopied(false), 1200);
    } catch {
      // clipboard blocked — fail silently, no broken UI
    }
  };
  ...
  useEffect(() => {
    ...
    if (!root || !canvasEl || skills.length === 0) return;
    ...
    try {
      ...
      console.warn('SkillsShowcase: WebGL renderer creation failed', err);
    ...
    // Warm dark reflection env (was cool navy/blue) so metal/glass highlights read as
    // "BRIJ dark UI" instead of a library-lounge blue.
    (function buildEnv() {
      ...
      envBlob(x, 140, 66, 95, '232,84,42', 0.75);
      envBlob(x, 405, 84, 55, '255,214,168', 0.35);
      envBlob(x, 256, 150, 120, '232,84,42', 0.22);
    ...
    const hemi = new THREE.HemisphereLight(0x8a7a68, 0x0a0908, 0.32);
    scene.add(hemi);
    ...
    scene.add(key);
    const fillLight = new THREE.DirectionalLight(0xffcba0, 0.2);
    fillLight.position.set(-4, 1, 4);
    scene.add(fillLight);
    const rim = new THREE.DirectionalLight(0xe8542a, 0.32);
    ...
    function striationTexture(vertical: boolean) {
      ...
      while (p < s) {
        const w = 1 + Math.random() * 2.4,
          tone = Math.random();
        x.fillStyle =
          tone < 0.12 ? 'rgba(232,84,42,.35)' : tone < 0.5 ? 'rgba(255,255,252,.12)' : 'rgba(120,110,95,.2)';
        if (vertical) x.fillRect(p, 0, w, s);
        else x.fillRect(0, p, s, w);
        p += w + 0.6 + Math.random() * 1.6;
      }
      for (let i = 0; i < 2600; i++) {
        x.fillStyle = 'rgba(0,0,0,' + (Math.random() * 0.15).toFixed(3) + ')';
    ...
    const endpaperTex = (function () {
      ...
      for (let i = 0; i < 1400; i++) {
        x.fillStyle = 'rgba(60,50,40,' + (0.04 + Math.random() * 0.08).toFixed(3) + ')';
    ...
    // Flat, high-contrast "chip" front cover — matches the hero section's
    // solid-orange / dark-outline chip motif instead of a paper book jacket.
    function paintSkillFront(
      x: CanvasRenderingContext2D,
      w: number,
      h: number,
      o: { name: string; category: string; bg: string; ink: string; border: string },
    ) {
      x.fillStyle = o.bg;
      x.fillRect(0, 0, w, h);
      ...
      x.globalAlpha = 0.5;
      x.textAlign = 'center';
      x.font = '700 54px monospace';
      x.fillText('{ }', w / 2, h * 0.28);
      x.globalAlpha = 1;

      x.font = '800 84px monospace';
      ...
      const startY = h * 0.5 - ((lines.length - 1) * 96) / 2;
      lines.forEach((l, i) => x.fillText(l, w / 2, startY + i * 96));

      x.globalAlpha = 0.6;
      x.font = '600 34px monospace';
      x.fillText(o.category.toUpperCase(), w / 2, startY + lines.length * 96 + 56);
      x.globalAlpha = 1;
    }

    // Back cover: fake terminal output instead of blurb-text bars + barcode.
    function paintSkillBack(
      x: CanvasRenderingContext2D,
      w: number,
      h: number,
      o: { backBg: string; backInk: string; yearsUsed: string },
    ) {
      x.fillStyle = o.backBg;
      x.fillRect(0, 0, w, h);
      const ink = o.backInk;

      const lines = ['$ skill --info', `> ${o.yearsUsed} in production`, '> status: active', '✓ ready'];
      x.textAlign = 'left';
      x.font = '500 34px monospace';
      let y = h * 0.34;
      lines.forEach((l, i) => {
        x.fillStyle = 'rgba(' + ink + ',' + (i === lines.length - 1 ? 0.9 : 0.55) + ')';
        x.fillText(l, 150, y);
        y += 62;
      });

      x.textAlign = 'right';
      x.font = '600 28px monospace';
      x.fillStyle = 'rgba(' + ink + ',.5)';
      x.fillText(o.yearsUsed.toUpperCase(), w - 150, h - 150);
      x.textAlign = 'left';
    }

    function paintSkillSpine(
      x: CanvasRenderingContext2D,
      w: number,
      h: number,
      o: { spineBg: string; spineInk: string; spineFont: string; name: string; category: string },
    ) {
      ...
      x.font = o.spineFont;
      drawSpaced(x, o.name.toUpperCase(), -h * 0.1, 15, 6);
      x.globalAlpha = 0.7;
      x.font = '600 25px monospace';
      drawSpaced(x, o.category.toUpperCase(), h * 0.325, 9, 4);
      x.globalAlpha = 1;
      x.restore();
      x.fillStyle = o.spineInk;
      x.globalAlpha = 0.6;
    ...
    // Interior page: "USE CASES" list instead of a book INDEX, no fake page numbers.
    function makeUseCasesPageTex(useCases?: string[]) {
      const w = 1024,
        h = 1536,
        c = mkCanvas(w, h),
        x = c.getContext('2d')!;
      x.fillStyle = '#ede9e3';
      x.fillRect(0, 0, w, h);
      x.fillStyle = 'rgba(10,10,10,0.05)';
      for (let i = 0; i < 1600; i++) x.fillRect(Math.random() * w, Math.random() * h, 1.1, 1.1);
      x.fillStyle = '#0a0a0a';
      x.textAlign = 'center';
      x.font = '800 76px monospace';
      x.fillText('USE CASES', w / 2, 190);
      x.globalAlpha = 0.26;
      x.fillRect(220, 225, w - 440, 3);
      x.globalAlpha = 1;

      const list = useCases && useCases.length ? useCases : ['Production use', 'Side projects', 'Learning'];
      x.textAlign = 'left';
      x.font = '500 44px monospace';
      let y = 318;
      for (let i = 0; i < list.length; i++) {
        const n = String(i + 1).padStart(2, '0');
        const left = n + '. ' + trimToWidth(x, list[i], 780);
        x.fillStyle = '#0a0a0a';
    ...
    const paperFlat = std({ color: 0x1c1a17, roughness: 0.95, envMapIntensity: 0.2 });
    ...
    const pageMats = [0x201d19, 0x1a1815, 0x25211d].map((c) =>
    ...
    const bookInstances: SkillInstance[] = [];
    const hitMeshes: THREE.Mesh[] = [];

    function buildBook(cfg: SkillCfg, index: number): SkillInstance {
      const root = new THREE.Group();
      const float = new THREE.Group();
      root.add(float);
      bookRoot.add(root);

      const indexPageMat = std({ map: makeUseCasesPageTex(cfg.useCases), roughness: 0.92, envMapIntensity: 0.2, side: THREE.DoubleSide });

      // Alternate solid-orange / dark-outline covers across the shelf, same
      // rhythm as the hero section's mixed chip styling.
      const isAccentBg = index % 2 === 1;
      const skillBg = cfg.spineBg ?? cfg.backBg ?? (isAccentBg ? '#e8542a' : '#0a0a0a');
      const skillInk = isAccentBg ? '#0a0a0a' : '#ffffff';
      const skillBorder = isAccentBg ? '#0a0a0a' : '#e8542a';

      const edgeColor = cfg.edge ?? '#3a332a';
      ...
      loadOrPaint(mFront, cfg.images?.front ?? null, () => {
        ...
        else paintSkillFront(ctx, 1024, 1536, { name: cfg.name, category: cfg.category, bg: skillBg, ink: skillInk, border: skillBorder });
        return c;
      });
      loadOrPaint(mBack, cfg.images?.back ?? null, () => {
        ...
        else paintSkillBack(ctx, 1024, 1536, { backBg: cfg.backBg ?? '#0a0a0a', backInk: cfg.backInk ?? '255,255,255', yearsUsed: cfg.yearsUsed });
        return c;
      });
      loadOrPaint(mSpine, cfg.images?.spine ?? null, () => {
        ...
        if (cfg.spine) cfg.spine(ctx, 220, 1536);
          ...
          paintSkillSpine(ctx, 220, 1536, {
            spineBg: skillBg,
            spineInk: cfg.spineInk ?? skillInk,
            spineFont: cfg.spineFont ?? '700 42px monospace',
            name: cfg.name,
    ...
    skills.forEach(buildBook);
    const bookByHit = (m: THREE.Object3D) => bookInstances.find((b) => b.hit === m)!;

    // Floating particles (detail view) — recolored orange/white sparks, was green leaves.
    const leaves = {
      items: [] as any[],
      anchor: null as SkillInstance | null,
      activate(book: SkillInstance) {
        this.anchor = book;
    ...
    (function buildLeaves() {
      ...
      const cols = [0xe8542a, 0xc94a24, 0xffffff, 0x8a8378];
      for (let i = 0; i < 16; i++) {
        const mat = std({ color: cols[i % 4], roughness: 0.4, envMapIntensity: 0.35, side: THREE.DoubleSide });
    ...
    const state: {
      ...
      selected: SkillInstance | null;
      hovered: SkillInstance | null;
      pillLock: SkillInstance | null;
      kbIndex: number;
    ...
    function setTargets(b: SkillInstance, slot: Slot) {
      const s = b.springs;
      s.px.t = slot.p[0];
      s.py.t = slot.p[1];
      s.pz.t = slot.p[2];
    ...
    function playY(b: SkillInstance, segs: any[]) {
      b.exit = { segs, i: 0, t: 0 };
    }
    function stepY(b: SkillInstance, dt: number) {
      const ex = b.exit!,
        s = b.springs;
      ex.t += dt;
    ...
    function pinInPlace(b: SkillInstance) {
      const s = b.springs;
      s.px.t = s.px.v;
      s.pz.t = s.pz.v;
      s.rx.t = s.rx.v;
    ...
    function sendOut(b: SkillInstance, i: number, delay: number) {
    ...
    function bringBack(b: SkillInstance, i: number, delay: number) {
    ...
    function open(book: SkillInstance | null) {
      ...
      onSkillSelectRef.current?.(book.cfg);
      computeSlots();
    ...
    let dragBook: SkillInstance | null = null,
      rayBook: SkillInstance | null = null;
    ...
    function animate(timestamp?: number) {
      ...
      let hov: SkillInstance | null = null;
      if (state.mode === 'hero') {
  ...
  }, [skills, showDetailPanel]);

  const themeVars = {
    '--bs-navy': themeColors?.navy ?? '#0a0a0a',
    '--bs-pink': themeColors?.pink ?? '#e8542a',
    '--bs-cream': themeColors?.cream ?? '#ede9e3',
    '--bs-lav': themeColors?.lav ?? '#b8b3ab',
    '--bs-peri': themeColors?.peri ?? '#c94a24',
    '--bs-bg-light': themeColors?.bgLight ?? themeColors?.bg ?? '#ede9e3',
    '--bs-bg-dark': themeColors?.bgDark ?? themeColors?.bg ?? '#0a0a0a',
    '--bs-fg-light': themeColors?.foregroundLight ?? '#0a0a0a',
    '--bs-fg-dark': themeColors?.foregroundDark ?? '#ede9e3',
  } as React.CSSProperties;

  const panelVisible = uiMode === 'detail';
  const heroWordVisible = mounted && uiMode === 'hero';
  const canCarousel = showCarousel && skills.length > 3;
  ...
  const proficiency = selectedCfg?.proficiency ?? 0;
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label={`${heroTitle} skill showcase`}
      data-state={uiMode}
      className={cn(
        'skill-showcase relative isolate h-full min-h-[560px] overflow-hidden font-mono outline-none [container-type:size] [-webkit-tap-highlight-color:transparent]',
    ...
    >
      ...
      {skills.length === 0 && (
        <div className="absolute inset-0 z-10 flex items-center justify-center p-8 text-center text-sm text-current opacity-60">
          Add at least one skill to display the showcase.
        </div>
      )}
      ...
      <button
        ...
        className={
          ...
          '[background:repeating-linear-gradient(92deg,rgba(232,84,42,0.06)_0px_2px,transparent_2px_6px),radial-gradient(125%_150%_at_28%_0%,#1a1a1a_0%,#111111_58%,#0a0a0a_100%)] ' +
          ...
          <div className={`mt-[34px] flex items-center gap-[18px] @max-[760px]:mt-[18px] ${dpChild(210)}`}>
            <div className="flex gap-[6px]">
              {[0, 1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className={`h-[16px] w-[16px] rounded-[3px] ${i < proficiency ? 'bg-[var(--bs-pink)]' : 'bg-[var(--bs-pink)]/20'}`}
                />
              ))}
            </div>
            <div className="h-6 w-px bg-[var(--bs-lav)]/[0.28]" />
            <div className="text-[15px] uppercase tracking-[0.08em] text-[#c9a99a]">Proficiency</div>
            <div className="ml-auto text-[15px] uppercase tracking-[0.08em] text-[#c9a99a]">{selectedCfg?.yearsUsed}</div>
          </div>
          <div className={`mt-[26px] border-t border-[var(--bs-lav)]/[0.18] @max-[760px]:mt-4 ${dpChild(270)}`} />
          <div
            className={`pointer-events-auto mt-8 inline-flex items-center gap-[10px] rounded-full bg-[#151312] p-[10px] shadow-[0_24px_60px_rgba(0,0,0,0.45)] @max-[760px]:mt-[18px] @max-[760px]:flex-wrap @max-[760px]:rounded-[28px] ${dpChild(330)}`}
          >
            <button
              onClick={() => selectedCfg && onViewProjects?.(selectedCfg)}
              className="inline-flex h-[54px] items-center gap-[10px] rounded-full bg-[var(--bs-cream)] px-[26px] text-[15px] font-bold uppercase tracking-[0.05em] text-[var(--bs-navy)] transition-[transform,filter] duration-[220ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-[1.04] hover:brightness-105 @max-[760px]:h-12 @max-[760px]:px-5 @max-[760px]:text-[13px]"
            >
              View Projects
            </button>
            <button
              onClick={handleCopy}
              className="inline-flex h-[54px] items-center gap-[10px] rounded-full bg-[var(--bs-peri)] px-[26px] text-[15px] font-bold uppercase tracking-[0.05em] text-[#0a0a0a] transition-[transform,filter] duration-[220ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-[1.04] hover:brightness-105 @max-[760px]:h-12 @max-[760px]:px-5 @max-[760px]:text-[13px]"
            >
            ...
            {selectedCfg?.docsUrl && (
              <a
                ...
                className="inline-flex h-[54px] w-[54px] items-center justify-center gap-[10px] rounded-full bg-[#242018] px-0 text-[var(--bs-lav)] transition-[transform,filter] duration-[220ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] hover:scale-[1.04] hover:brightness-105"
              >
                <DocsIcon />
              </a>
            )}
